
Ext.define("acesure.view.sysLog.SystemLogToolbar", {
	extend : 'Ext.Toolbar',
	alias : 'widget.systemLogToolbar',
	height:108,
	padding:'0 0 0 25',
	style:'border:0;border-bottom:1px solid #d1dade;background:#fafbfc;',
	defaults:{bodyStyle:'background:#fafbfc'},
	items : [ {
						xtype : "panel",
						border : false,
						width:48,
						height:42,
						html : '<img src="/images/log/log_system_title.png"/>'
				 }, {
						xtype : "panel",
						border : false,
						html : "<font class='font_h3'>"+local.log.titleSystemLog+"</font>"
				 },
				 "->",
				 {
				xtype : "button",
				text : local.btn.refresh,
				style:'padding-left:26px',
				icon : '/images/common/refresh.png',
				handler : function() {
					var systemLogGridId = Ext.getCmp("systemLogGridId");
					systemLogGridId.getStore().reload();
				}
			}, {
				xtype : "button",
				text : local.btn.export0, 
				style:'padding-left:26px',
				icon : '/images/log/export.png',
				id : 'systemLogExportId',
				itemId : 'reportlog_log_sysexport'
			}, {
				xtype : "panel",
				id : 'systemLogShrink',
				border:false,
				width:56,
				margin:0,
				height:"100%",
				html: "<div class='shrink' onclick='fadeout();'><img src='/images/backup/shrink.png'/></div>"
			}]
});

//返回事件
function fadeout() {
	var reportLogPanel = Ext.getCmp('reportLogPanel');
	reportLogPanel.removeAll();
	reportLogPanel.add({
		xtype : 'logView'
	});
	reportLogPanel.doLayout();
}

Ext.define("acesure.view.log.report.SystemCenterToolbar", {
	extend : 'Ext.Toolbar',
	alias : 'widget.systemCenterToolbar',
	height: 58,
	padding:20,
	border:false,
	items : [ '<font class="font_t">'+local.log.titleSystemLog+'</font>', "->" ,{
				xtype:'combobox',
				store : new Ext.data.SimpleStore({
					fields : ['value', 'text'],
					data : [[1, local.normal], [2, local.defaulty],[3, local.warn]]
				}),
				displayField : "text",    //显示的
				valueField : "value",    //需要提交的值
				forceSelection : true,
				typeAhead : true,
				editable: false,
		        id: 'systemLogLevel',
		        labelWidth: 40,
		        fieldLabel: local.gridLevel,
		        width: 150
			},{
				xtype: 'datefield',
                width: 170,
                format :'Y-m-d',
                editable : false,
                labelWidth: 40,
                fieldLabel: local.time,
                id: 'beginDateId'
			}, ' - ',{
				xtype: 'datefield',
                width: 120,
                format :'Y-m-d',
                editable : false,
                id: 'endDateId'
			}, {
				xtype : "button",
				text : local.btn.inquire,
				id: 'searchSystemLogBut'
			},{
				xtype : "button",
				text :local.log.reset,
				id: 'resetSystemLogBut'
			} ]
});

	 
Ext.define('acesure.view.sysLog.SystemLogGrid', {
		extend:'Ext.grid.Panel',
	    xtype: 'row-expander-grid',
	    alias:'widget.systemLogGrid',
	    id: 'systemLogGridId',
	    store: 'sysLog.SystemLogStore',
	    border:false,
		enableColumnResize:false,
		enableColumnMove:false,
		enableColumnHide:false,
	    margin:'10 20 20 20',
	    style:'border:1px solid #e3eaec;',
	    columns: [ 
			{ header:local.num, menuDisabled:true,dataIndex: 'systemLogId',width:80 },
			{ header: local.log.eventId, menuDisabled:true,width:90,
				renderer:function(v) {
					return "30000";
				}
			},
			{ header:local.log.gridLevel, menuDisabled:true,dataIndex: 'systemLogLevel', width:100, 
				renderer:function(v) {
					if(v == 1){
						return "<img src='/images/log/normal.gif' align='absmiddle' />&nbsp;"+local.normal;
					}else if(v == 2){
						return "<img src='/images/log/error.gif' align='absmiddle' />&nbsp;"+local.defaulty;
					}else if(v == 3){
						return "<img src='/images/log/warning.gif' align='absmiddle' />&nbsp;"+local.warn;
					}
				}
			},
			{ header: local.log.gridSource, menuDisabled:true,tdCls:"font_color_999",dataIndex: 'systemLogManager' , flex : 1 },
			{ header:local.type, tdCls:"font_color_999",menuDisabled:true,dataIndex: 'systemLogType' , width:150,
				renderer:function(v){
					if(v == 1){
						return local.log.AccOpeLog;
					}else{
						return local.log.moduleRun;
					}
				}
			},
			{ header: local.time, tdCls:"font_color_999",menuDisabled:true,dataIndex: 'systemLogInsertTime' , width:200 },
			{ header: local.backup.content,tdCls:"font_color_999", menuDisabled:true,dataIndex: 'systemLogContent' , flex: 2,
				renderer : function(v,m,record,ri,ci) {
					return "<div title='" + v + "'>" + v + "</div>";
				}
			}
		],
	    listeners : {
	    	afterrender : function(){
	    		Ext.getCmp("systemLogGridId").store.load();
	    	}
	    },
	    dockedItems: [{
			id:'systemLogPage',
	        xtype: 'pagingtoolbar',
	        store: 'sysLog.SystemLogStore',
	        dock: 'bottom', //分页 位置
	        emptyMsg:local.toobarEmptyMsg,
            displayInfo: true,
            displayMsg:local.toolbarDisplayMsg,
            beforePageText:local.toolbarBeforePageText,
            afterPageText:local.toolbarAfterPageText	
		}]
	});

Ext.define('acesure.view.sysLog.SystemLogList', {
	extend : 'Ext.panel.Panel',
	width : '100%',
	border : false,
	alias : 'widget.systemLogList',
	layout:"vbox",
	overflowY:"auto",
	items: [{
		width:"100%",
		xtype : 'systemLogToolbar'
	},{
		width:"100%",
		xtype : 'systemCenterToolbar'
	},{
		width:"100%",
		flex:1,
		minHeight:200,
		maxHeight:563,
		xtype : 'systemLogGrid'
	}],
	listeners:{
		render:function(v, eOpts){
			POWER_OP.filterEnableWidgetsOfExtjs(v, CURRENT_USER.getReportlogPower());
		}
	}
});