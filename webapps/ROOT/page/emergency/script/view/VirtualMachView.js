...
Ext.define('acesure.emergency.view.virtualTabPanel', {
	...
	items : [ {
		...
		title : '附件设置',
		items: {xtype : "attachmentPanel",width : '100%',id:'attachmentPanelId'},
		listeners: { activate: showVirAttachment }
	} ]
});
 ...
 * auth:zjc
 */
Ext.define("acesure.emergency.view.attachmentPanel",{
	extend:"Ext.Panel",
	...
	items:[
	       {
	    	   ...
	    	   width:"100%",
	    	   overflowY:"auto",
	    	   html:"<span id='span_attachmentPanel'></span>"
	       }]
});
	          ...
	          },
	          {
	        	  ...
	        	  renderer : function(v, p, r) {
	        		  ...
	        		  if(mergeStatus==0){
	        			  ...
	        			  return "<img src='/images/common/green.png' /><span style='font-weight:bold;color:#2db200'>"+local.emergency.creatingRate+mergePercent+"%</span>";
	        			  
	        		  }else if(mergeStatus==2){//小颗粒合并失败
	        			  return "<img src='/images/common/gray.png' /><span style='font-weight:bold;color:red'>"+local.emergency.mercyFailure+"</span>";
	        			  ...
	        			  if(v==1){
		        			  ...
		        			  return "<img src='/images/common/gray.png' /><span style='font-weight:bold;color:#ccc'>"+local.emergency.mercyFinishUnstart+"</span>";
		        		  }else{
		        			  return "<img src='/images/common/gray.png' /><span style='font-weight:bold;color:#ccc'>"+local.unconfig+"</span>";
		        		  }
	        		  }
	        		  ...
	        		  if(SERVERANME){
	        			  ...
	        			  var storeList=this.store.load({callback:function(records, operation, success){
	        				  ...
	        				  for(i=0;i<records.length;i++){
	        					  ...
	        					  //2017/4/17 YangBobin update 小颗粒主机名称小颗粒主机页面虚拟机显示名称如 Server_4[2017-04-14 23:24:08]
                                  var vmShowName = tRecord.get("vmName");  //虚拟主机显示名称
                                  if(vmShowName.indexOf('[')>0){
                                    vmShowName = vmShowName.substring(0,vmShowName.indexOf('['));  //Server_4
                                  }
                                  
                                  if(SERVERANME==vmShowName){
                                      record=tRecord;
                                  }
	        				  };
	        				  if(record){
 ...
 * auth:wangshaodong
 */
Ext.define("acesure.emergency.view.VirtualMachView",{
	extend:"Ext.Panel",
	alias:"widget.VirtualMachView",
	border:false,
	overflowY:"auto",
	layout:"vbox",
	items:[
	       {
	    	   ...
	    	   padding:"20 20 10 20",
	    	   height:55,
	    	   //html : "<font class='font_t'>"+local.recovery.virtualData+"</font>",
	    	   html : "<font class='font_t'>"+local.emergency.vmList+"</font>",
	    	   width : '100%'
	       },
...
Ext.define("acesure.emergency.view.updateVmNicConfigWindow", {
	...
	buttons : [{
		text :local.btn.save,
		cls:"btn_focus",
		id:"updateVmNicConfigWindowSaveBtn",
		handler : function() {
...
/**
 * 改变保存按钮的状态
 * @param state 0：置灰 1:开启
 */
function changeBtnState(state){
	var saveBtn = Ext.getCmp("updateVmNicConfigWindowSaveBtn");
	if(saveBtn && state){
		saveBtn.enable();
	} else {
		saveBtn.disable();
					...
					items:[
					       {
					    	   width:120,
					    	   xtype:"label",
					    	   html:local.emergency.deviceOfflineTime
					       },
					       {
					    	   xtype:'combobox',
					    	   id:'Interrupt-min-id',
		...
		buttons:[
		         {
		        	text:local.btn.save,cls:"btn_focus",handler:function(v){
		        		...
		        		//保存警告框
		        		Ext.MessageBox.show({
		                    title: local.emergency.confirmSaveAutoTakeOverConfig,
		                    msg: local.emergency.confirmSaveAutoTakeOverConfigMsg,
		                    buttons: Ext.MessageBox.YESNO,
		                    buttonText:{ 
		                        yes: local.btn.confirm, 
		                        no: local.btn.cancle
		                    },
 ...
 * auth:lidongsheng
 */
Ext.define("acesure.emergency.view.updateVmAttachmentWindow", {
	extend : 'Ext.window.Window',
	title: "增加或修改虚拟机附件", 
	draggable: false,
	width: 450,
	id:'updateVmAttachmentWindow',
	layout: "vbox",
	modal: true, 
	resizable: false,
	attachment:{
...
//    	   labelCls:"label_icon label_icon_cal",
		...
		items : {
			...
			columns: 1,
			items:[
			       { id:'vmUSB1',boxLabel:'USB 类型1' ,name:'vmUSBType',inputValue:1},
			       { id:'vmUSB2',boxLabel:'USB 类型2' ,name:'vmUSBType',inputValue:2 },
			       { id:'vmUSB3',boxLabel:'USB 类型3' ,name:'vmUSBType',inputValue:3 ,checked: true}
			       ],
			       listeners:{
			    	   'afterrender':function(me, opts){
			    		   var virtualTabPanel=Ext.getCmp("virtualTabPanel");
			    		   var vmId=virtualTabPanel.virtualIndex;
			    		   var box = Ext.getCmp("usbListId");
			    		   emergencyMapDate.getAllAttachmentList(vmId, box);
			    		   // 没有选择的时候保存按钮置灰
			    		   chgAttachmentBtnState(0);
			    	   }
			       }      
		}
	}],
	buttons : [{
		...
		handler : function() {
			// 获取combox ID值
			var id = Ext.getCmp('usbListId').getValue();
			// 获取显示值
			var information = Ext.getCmp('usbListId').getRawValue();
			var usbType = Ext.getCmp("vmUSBTypeId").getChecked()[0].inputValue;
			...
			var updateAttachmentWindow = Ext.getCmp("updateVmAttachmentWindow");
			var oldAttachment = updateAttachmentWindow.attachment;
			var createType = updateAttachmentWindow.createType;
			console.debug("createType", createType);
			
			if(createType == 2){
				// update
				console.info("updateAttachment");
				updateVmAttachment(oldAttachment, attachment, usbType);
			} else if (createType == 1) {
				// add
				console.info("addAttachment");
				addVmAttachment(id, information, usbType);
			}
			this.up('window').close();
		}
	},{
...
// 附件保存按钮显示与置灰 1:显示 0：置灰
function chgAttachmentBtnState(state){
	var btn = Ext.getCmp("attachmentSaveId");
	if(btn && state){
		btn.enable();
	} else {
		btn.disable();
